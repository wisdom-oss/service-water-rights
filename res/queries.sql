...
FROM nlwkn_water_rights.e_usage_locations
WHERE ST_CONTAINS(ST_Union(ARRAY((SELECT geom FROM geodata.shapes WHERE key = any($2)))), st_transform(location, 4236));

-- name: get-water-rights-by-reality-and-location
SELECT id, water_right, active, real, name, ST_ASGEOJSON(st_transform(location, 4236)) as location
FROM nlwkn_water_rights.e_usage_locations
WHERE (real = $1 OR real IS NULL)
AND ST_CONTAINS(ST_Union(ARRAY((SELECT geom FROM geodata.shapes WHERE key = any($2)))), st_transform(location, 4236));
